...
RUST_SIMPLE = PROJECT_ROOT / "target" / "debug" / "examples" / "simple"
...
def run_rust_pikchr(test_file: Path) -> tuple[str, bool]:
    """Run the Rust pikchr implementation on a test file."""
    if not RUST_SIMPLE.exists():
        return f"ERROR: Rust binary not found at {RUST_SIMPLE}. Run: cargo build --example simple", True

    try:
        result = subprocess.run(
            [str(RUST_SIMPLE), str(test_file)],
            capture_output=True,
            timeout=10
        )
        ...
        if result.returncode != 0:
            # Check if it's a parse error (expected for some tests)
            if "Error:" in stderr:
                return f"ERROR: {stderr}", True
            return f"ERROR: {stderr}", True

        # stdout contains the SVG, stderr has tracing logs
        return stdout, False
    except subprocess.TimeoutExpired:
...
def calculate_ssim(c_png: bytes | None, rust_png: bytes | None) -> float | None:
    ...
    if not HAS_SKIMAGE:
        logger.warning("scikit-image not available for SSIM calculation")
        return None
    if not c_png or not rust_png:
        logger.warning(f"Missing PNG for SSIM: c_png={len(c_png) if c_png else None}, rust_png={len(rust_png) if rust_png else None}")
        return None

    try:
        c_img = PILImage.open(io.BytesIO(c_png)).convert("RGB")
        rust_img = PILImage.open(io.BytesIO(rust_png)).convert("RGB")

        logger.info(f"SSIM input sizes: C={c_img.size}, Rust={rust_img.size}")
        ...
        diff = np.abs(c_array.astype(float) - rust_array.astype(float))
        logger.info(f"Pixel diff stats: mean={diff.mean():.2f}, max={diff.max():.2f}, nonzero={np.count_nonzero(diff)}")

        # channel_axis=-1 for RGB images (last axis is color channels)
        # data_range=255 for 8-bit images
        score = ssim(c_array, rust_array, channel_axis=-1, data_range=255)
        logger.info(f"SSIM score: {score}")
        # Return full-precision score so very small differences
        # (e.g. 0.9998 vs 1.0) are preserved for callers.
        return float(score)
    except Exception as e:
...
def run_pikru_test(test_name: str) -> list:
    ...
    # Generate fresh SVGs from both implementations
    c_output, c_is_error = run_c_pikchr(source)
    c_svg = extract_svg(c_output) if not c_is_error else None
    logger.info(f"C SVG: error={c_is_error}, len={len(c_svg) if c_svg else 0}")

    rust_output, rust_is_error = run_rust_pikchr(test_file)
    rust_svg = extract_svg(rust_output) if not rust_is_error else None
    logger.info(f"Rust SVG: error={rust_is_error}, len={len(rust_svg) if rust_svg else 0}")

    # Convert to PNGs
    c_png = svg_to_png(c_svg) if c_svg else None
    rust_png = svg_to_png(rust_svg) if rust_svg else None
    logger.info(f"PNGs: c={len(c_png) if c_png else 0}, rust={len(rust_png) if rust_png else 0}")
